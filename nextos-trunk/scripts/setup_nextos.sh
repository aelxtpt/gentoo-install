...
STATE_DIR="/var/lib/nextos"

remove_desktops() {
  local state_file="$STATE_DIR/desktops_removed"

  if [[ -f "$state_file" ]]; then
    einfo "Desktop removal already done; skipping"
    return
  fi
  ...
  emerge --quiet --unmerge \
    ...
    kde-plasma/plasma-meta kde-plasma/sddm kde-plasma/kdeplasma-addons kde-apps/kdecore-meta \
  ...
  einfo "Desktop removal complete; state saved"
}
...
sync_dots() {
  ...
  src="$dir/ui/dots-hyperland/dots/.config"
  if [[ -d "$src" ]]; then
...
create_quickshell_venv() {
  local user="$1" home venv_dir
  home="$(eval echo "~$user")"
  venv_dir="$home/.local/state/quickshell/.venv"

  if [[ -d "$venv_dir" ]]; then
    einfo "Quickshell venv already exists"
    return
  fi

  einfo "Creating Quickshell Python virtual environment"
  mkdir -p "$(dirname "$venv_dir")"
  sudo -u "$user" python3 -m venv "$venv_dir"
  chown -R "$user":"$user" "$home/.local/state/quickshell"
}

install_cursor_theme() {
  local user="$1" home dir cursor_src
  home="$(eval echo "~$user")"
  dir="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
  cursor_src="$dir/assets/cursors/NextOS"

  if [[ ! -d "$cursor_src" ]]; then
    ewarn "Cursor theme not found at $cursor_src; skipping"
    return
  fi

  # Install to both system and user directories for compatibility
  einfo "Installing NextOS cursor theme"
  mkdir -p /usr/share/icons "$home/.icons"
  rsync -a --delete "$cursor_src"/ /usr/share/icons/NextOS/
  rsync -a --delete "$cursor_src"/ "$home/.icons/NextOS"/
  chown -R "$user":"$user" "$home/.icons/NextOS"

  # Apply cursor immediately if Hyprland is running
  if pgrep -x Hyprland >/dev/null 2>&1; then
    local uid xdg_runtime hypr_sig
    uid="$(id -u "$user")"
    xdg_runtime="/run/user/$uid"
    hypr_sig="$(ls "$xdg_runtime/hypr/" 2>/dev/null | head -1)"

    if [[ -n "$hypr_sig" ]]; then
      einfo "Applying cursor theme"
      # Use size change workaround (hyprland bug: theme only changes if size changes)
      sudo -u "$user" XDG_RUNTIME_DIR="$xdg_runtime" HYPRLAND_INSTANCE_SIGNATURE="$hypr_sig" \
        hyprctl setcursor NextOS 25 2>/dev/null || true
      sudo -u "$user" XDG_RUNTIME_DIR="$xdg_runtime" HYPRLAND_INSTANCE_SIGNATURE="$hypr_sig" \
        hyprctl setcursor NextOS 24 || ewarn "hyprctl setcursor failed"
    else
      ewarn "Could not find Hyprland instance; cursor will apply on next login"
    fi
  fi
}
...
main() {
  ...
  enable_seatd
  add_groups "$user"
  install_cursor_theme "$user"
  create_quickshell_venv "$user"
  sync_dots "$user"
  build_quickshell "$user"
  install_material_symbols "$user"

  einfo "Done. Reboot or relogin as $user, then run ~/start.sh to launch NextOS."
}
